...
async def add_symbol_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    指令: /add SOL 或 /add SOL/USDT
    添加新的监控标的（不会替换已有的）
    """
    args = context.args
    if not args:
        await update.message.reply_text("❌ 请输入代币名称，例如: `/add SOL`", parse_mode='Markdown')
        return
...
async def del_symbol_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target == "ALL":
        ...
        await update.message.reply_text(f"🛑 已停止所有监控（共 {count} 个）\n发送 `/add <币种>` 重新开始。", parse_mode='Markdown')
    else:
        # 删除指定标的
        if "/" not in target:
        ...
        if engine.remove_symbol(target):
            ...
            if remaining:
                ...
                await update.message.reply_text(f"🛑 已停止监控 `{target}`\n📋 剩余: {symbols_list}", parse_mode='Markdown')
            else:
                await update.message.reply_text(f"🛑 已停止监控 `{target}`\n💤 当前无监控目标", parse_mode='Markdown')
        else:
            await update.message.reply_text(f"❌ `{target}` 不在监控列表中", parse_mode='Markdown')
...
async def list_symbols_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not current_symbols:
        await update.message.reply_text("💤 当前没有正在监控的标的。\n请使用 `/add BTC` 添加。", parse_mode='Markdown')
        return
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not symbols:
        await update.message.reply_text("💤 当前未监控任何标的。\n请使用 `/add BTC` 开始。", parse_mode='Markdown')
        return
    ...
    if target_symbol:
        if target_symbol not in symbols:
            await update.message.reply_text(f"❌ `{target_symbol}` 不在监控列表中", parse_mode='Markdown')
            return
        symbols_to_query = [target_symbol]
    else:
        symbols_to_query = symbols

    loading_msg = await update.message.reply_text(f"🔄 正在分析 {len(symbols_to_query)} 个标的 (1H)...")
