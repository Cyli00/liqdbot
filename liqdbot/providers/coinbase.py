...
class CoinbaseProvider(DataProvider):
    """Coinbase数据提供者 - 用于获取BTC/USD等现货数据"""
    ...
    async def fetch_ohlcv(
        ...
        try:
            ...
            if duration_ms > SLOW_THRESHOLD_MS:
                logger.warning(
                    f"event=fetch_ohlcv_slow provider=coinbase symbol={symbol} "
                    f"tf={timeframe} limit={limit} duration_ms={duration_ms} rows={rows}"
                )
            else:
                logger.debug(
                    f"event=fetch_ohlcv provider=coinbase symbol={symbol} "
                    f"tf={timeframe} limit={limit} duration_ms={duration_ms} rows={rows}"
                )
            return df
            ...
            duration_ms = (time.perf_counter_ns() // 1_000_000) - start_ms
            logger.exception(
                f"event=fetch_ohlcv_error provider=coinbase symbol={symbol} "
    ...
    async def fetch_ticker(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            logger.exception(
                f"event=fetch_ticker_error provider=coinbase symbol={symbol} err={e}"
            )
            return None
    ...
    async def validate_symbol(self, symbol: str) -> bool:
        try:
            ...
            logger.exception(
                f"event=validate_symbol_error provider=coinbase symbol={symbol}"
            )
            return False
